...
export default function StatsPanel() {
    ...
    const stats = useMemo(() => {
        ...
        // إجمالي نقاط المسار المكتملة
        const totalRoutePoints = vehicles.reduce((sum, v) => sum + v.route.length, 0);
        const completedPoints = vehicles.reduce((sum, v) => {
            return sum + (currentPositionIndex[v.id] ?? 0);
        }, 0);

        // نسبة الإتمام الكلية
        const overallProgress = totalRoutePoints > 0
            ? Math.round((completedPoints / totalRoutePoints) * 100)
            : 0;
